...
// мокаем, чтобы проверять вызов, а не состояние localStorage. Само SMS
// отправляет OTP-страница (единый отправитель), не эта форма.
...
test('shows a validation error and does not navigate for an invalid phone', async () => {
  render(<PhoneForm />);
...
test('shows the consent error and does not navigate without the personal data checkbox', async () => {
  render(<PhoneForm />);
...
test('saves consents and redirects to /auth/otp with the phone on valid submit', async () => {
  render(<PhoneForm />);
