# Handoff — состояние проекта на 2026-07-14 (обновлено: FE-6 Оплата реализован)
...
- **Следующие кандидаты по критическому пути** (frontend-plan.md §11): FE-5 Чат (WebSocket, real-time + офлайн-очередь — крупнее предыдущих задач), либо FE-7 Уведомления (S, 1 нед., проще); FE-8 Admin-панель дальше по плану
- **Не поднято**: bundlewatch/lighthouse-ci (следующие CI-гейты), e2e Playwright-сценарии (FE-10)
- **Расхождение в доках, не устранено**: `frontend-plan.md` ссылается на оплату как FR-14, но в PRD.md FR-14 — «Рекомендательная система (ML)»; реальный номер требования оплаты — FR-10. Стоит поправить при следующей правке frontend-plan.md
